...
/**
 * EasyUI DataGrid 服务端配置模型。
 *
 * <p>由 {@code @UIDataGrid} 注解通过 {@code AbstractComponentBuilder.buildComponentConfig}
 * 自动转换而来，序列化为 JSON 返回客户端渲染为 EasyUI DataGrid。</p>
 *
 * <p>支持的配置项包括：分组字段、单选/多选、分页、冻结列、工具栏、右键菜单等。</p>
 *
 * @see XUITreeGrid
 * @see GridColumn
 */
public class XUIDataGrid extends GridComponent {
	...
	public XUIDataGrid(UIDataGrid uiGrid, String builder) {
		this(uiGrid);
	...
	public XUIDataGrid(UIDataGrid uiGrid) {
		...
		if (FlameUtils.isNotBlank(uiGrid.groupField())) {
			this.setShowGroup(true);
			this.setGroupField(uiGrid.groupField());
		}
		...
		for (UIColumn uicolumn : uiGrid.columns()) {
			...
			/**
			 * 若当前非hidden/checkbox字段, 并且在title属性中未设置值, 则需要从资源文件中获取多语言信息;
			 */
			...
			column.setExpander(uicolumn.expander());
			if (uicolumn.expander()) {
	...
	public void addFrozenColumn(List<GridColumn> frozenColumns) {
		this.frozenColumns.add(frozenColumns);
	}

	public void addFrozenColumn(GridColumn column) {
		if (this.frozenColumns.isEmpty()) {
			this.frozenColumns.add(new ArrayList<>());
		}
		List<GridColumn> frozens = this.frozenColumns.get(0);
		frozens.add(column);
	}
	...
	public void addColumn(GridColumn column) {
		...
		List<GridColumn> columns = this.columns.get(0);
		columns.add(column);
	}
