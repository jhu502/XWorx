        ...
        public Object executeCall(Object[] arguments) {
            ...
            if (JSGuards.isForeignObject(receiver)) {
                ...
                if (optimistic) {
                    try {
                    	/**
                    	 * 判断receiver是否是HostObject，然后判断是否包装IThingEntity对象，若是转去调用invoke
                    	 */
                    	if (receiver instanceof HostObject) {
                    		HostObject hostObj = (HostObject) receiver;
                    		if (hostObj.getObject() instanceof IThingManaged) {
                    			callReturn = interop.invokeMember(receiver, "invokeService", new Object[] { functionNameJavaString, callArguments });
                    		} else {
                    ...
                    if (interop.isMemberInvocable(receiver, functionNameJavaString)) {
                        try {
                        	...
                        	if (receiver instanceof HostObject) {
                        		HostObject hostObj = (HostObject) receiver;
                        		if (hostObj.getObject() instanceof IThingManaged) {
                        			callReturn = interop.invokeMember(receiver, "invokeService", functionNameJavaString, callArguments);
                        		} else {
                        			callReturn = interop.invokeMember(receiver, functionNameJavaString, callArguments);
                        		}
                        	} else {
