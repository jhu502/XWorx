...
public final class HostObject implements TruffleObject {
    ...
    Object readMember(String name,
        ...
        /**
         * 在“Object readMember(String name,...)”方法最后添加：
         *  1. 当javascript代码通过object.name的方式去获取属性时，如果object无name属性，则去判断object是否是IThingEntity对
         *     象，如果是IThingEntity对象，自动调用readProperty()方法去获取值；
         *  2. 也就是说：从IThingEntity中读取ThingModel定义的PropretyDefinition的值
         */
        if (this.obj instanceof IThingManaged) {
        	return ((IThingManaged<?>) this.obj).readProperty(name);
        } else {
            error.enter(node);
    ...
    void writeMember(String member, Object value,
    	...
    	/**
    	 * 写ThingModel定义的PropretyDefinition的值到IThingEntity
    	 */
    	if (this.obj instanceof IThingManaged) {
    		((IThingManaged<?>) this.obj).writeProperty(member, value);
    	} else {
	        if (isNull()) {
	            error.enter(node);
	            throw UnsupportedMessageException.create();
